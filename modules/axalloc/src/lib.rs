...
use allocator::{BaseAllocator,PageAllocator,ByteAllocator};
...
impl GlobalAllocator {
    ...
    pub fn alloc(&self, layout: Layout) -> AllocResult<NonNull<u8>> {
        // debug!("GlobalAllocator try alloc LayOut{:?}",layout);
        let x = self.inner.lock().alloc(layout);
        // if let Ok(ok) = x {
            // debug!("GlobalAllocator alloc layout at {:?} successfully",ok);
        // }else{
            // debug!("GlobalAllocator alloc layout at fail");
        // }
        x
    }
    ...
    pub fn dealloc(&self, pos: NonNull<u8>, layout: Layout) {
        // debug!("GlobalAllocator try dealloc LayOut{:?} at pos {:?}",layout,pos);
    ...
    pub fn alloc_pages(&self, num_pages: usize, align_pow2: usize) -> AllocResult<usize> {
        // debug!("GlobalAllocator try alloc num_pages:{:x},page_size:{:x}",num_pages,align_pow2);
        let x = self.inner.lock().alloc_pages(num_pages, align_pow2);
        // if let Ok(ok) = x {
        //     debug!("GlobalAllocator alloc page at {:x} successfully",ok);
        // }else{
        //     debug!("GlobalAllocator alloc page at fail");
        // }
        x
    }
    ...
    pub fn dealloc_pages(&self, pos: usize, num_pages: usize) {
        // debug!("GlobalAllocator try dealloc num_pages:{:x},pos:{:x}",num_pages,pos);
